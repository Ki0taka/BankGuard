...
export class EncryptionService {
    ...
    constructor(private configService: ConfigService) {
        ...
        if (!keysString) {
            this.logger.warn('ENCRYPTION_KEYS not found in config. Using fallback key for dev only.');
            ...
            if (this.keys.length === 0 || this.keys[0].length !== 32) {
                throw new Error('Invalid ENCRYPTION_KEYS. Must be comma separated string of 32-byte keys.');
            }
        }
    }
